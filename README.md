![Typing SVG](https://readme-typing-svg.herokuapp.com/?lines=Aspiring+Data+Analyst;SQL+Python+Power+BI;PL-300+%26+DP-700+Certified)
## Hi there 👋 I'M JEEVAN S J! I'M SO GLAD THAT YOU ARE HERE!

💡 Welcome to my GitHub profile  
🚀 Passionate about Data Analytics & Tech  

---

## 🔧 Projects
- 📊 Power BI dashboards
- 🧹 Data Cleaning (SQL + Python)
- 📈 KPI Analysis Projects
- Domain Project (SQL+PowerBI)
- Tableau Dashboard Projects
- Excel project
- Python Project
- Python Libraries
## 🚀 Projects

### 📊 Sales Dashboard (Power BI)
- Built an interactive dashboard to analyze sales performance across regions
- Created KPIs like Total Revenue, Profit, and Growth %
- Used DAX for calculated measures and insights
- Tools: Power BI, Excel

---

### 🧹 Data Cleaning & Analysis (Python)
- Cleaned raw dataset with missing values and duplicates
- Performed exploratory data analysis (EDA)
- Visualized trends using charts and graphs
- Tools: Python, Pandas, NumPy, Matplotlib, Seaborn

---

### 🗄️ SQL Business Insights Project
- Wrote complex SQL queries using joins, subqueries, and window functions
- Generated insights like top customers, revenue trends, and product performance
- Tools: MySQL

---

### 📈 Tableau Dashboard Project
- Created interactive dashboards with filters and drill-down features
- Visualized business KPIs and trends
- Tools: Tableau
---

## 📚 Learning
- Azure (DP-700)
- PL-300(Microsoft PowerBI)

---

## ⚙️ Tech Stack

### 🧑‍💻 Languages & Libraries
![Python](https://img.shields.io/badge/Python-blue?style=for-the-badge&logo=python)
![NumPy](https://img.shields.io/badge/NumPy-blue?style=for-the-badge&logo=numpy)
![Pandas](https://img.shields.io/badge/Pandas-black?style=for-the-badge&logo=pandas)
![Matplotlib](https://img.shields.io/badge/Matplotlib-orange?style=for-the-badge)
![Seaborn](https://img.shields.io/badge/Seaborn-lightblue?style=for-the-badge)

### 🗄️ Data & Tools
![SQL](https://img.shields.io/badge/SQL-grey?style=for-the-badge&logo=mysql)
![Excel](https://img.shields.io/badge/Excel-green?style=for-the-badge&logo=microsoft-excel)

### 📊 Visualization
![Power BI](https://img.shields.io/badge/Power%20BI-yellow?style=for-the-badge&logo=powerbi)
![Tableau](https://img.shields.io/badge/Tableau-blue?style=for-the-badge&logo=tableau)

### ☁️ Cloud
![Azure](https://img.shields.io/badge/Azure-blue?style=for-the-badge&logo=microsoft-azure)

- SQL
- Python
- Power BI
- Excel
- Azure
- Tableau
- Numpy
- Pandas
- MatplotLib
- SeaBorn

---

## 📊 GitHub Stats
![Stats](https://github-readme-stats.vercel.app/api?username=jeevansj2116&show_icons=true&theme=tokyonight)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=jeevansj2116&layout=compact)
